...
const Pricing = ({data}) => {
  ...
  useEffect(()=>{
    ...
    const fetchData = async()=>{
     try{ const myHeaders = new Headers();
        ...
        const response = await fetch(`https://api.apilayer.com/currency_data/convert?to=${currencyDetails.toCurrency}&from=${currencyDetails.fromCurrency}&amount=${currencyDetails.amount}`, requestOptions)
...
//Note that in this version, I am using the Object.keys() method to get an array of the object's own enumerable properties, and then using the Array.prototype.map() method to create an array of <option> elements. This approach is more concise and easier to read than using a for...in loop.
  const currencyList = (list, selectedOption, name ) => (
    <select className="bg-purple-800 text-white text-lg p-2 rounded-lg m-1 w-[40%] truncate" value={selectedOption} onChange={handleChange} name={`${name}`}>
      
      {Object.keys(list).map(prop => (

        <option value={prop} key={prop} >{prop}: {list[prop]}</option>
      ))}
    </select>
  );
      ...
      >
        ...
        <div className="grid gap-6 my-8 sm:grid-cols-2 lg:mx-32">
          <div className={styles.glass}>
            ...
            placeholder={`${currencyDetails.amount}${"    "}change the amount here`}
            name='amount'
        ...
        <div className="grid place-items-center">
            <button className="bg-purple-800 text-white text-lg p-2 rounded-lg m-1">Convert</button>
          </div>
          <p className="text-xl text-gray-200  mx-4 my-14 text-center">
          {currencyDetails.amount}{" "}{currencyDetails.fromCurrency} = {result}{" "}{currencyDetails.toCurrency}
        </p>
      </form>
